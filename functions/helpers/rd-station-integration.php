...
function addLeadConversionToRdstationCrm( $rdstation_token, $identifier, $data_array ) {
	...
	$filename = './logs/'.time();
	file_put_contents($filename.'-rd-response', 'Trying to run at least');
	...
	try {
		...
		if (empty($data_array["email"])) { $data_array["email"] = $data_array["client-email"]; }
		if (empty($data_array["name"])) { $data_array["name"] = $data_array["client-name"]; }
		if (empty($data_array["mobile_phone"])) { $data_array["mobile_phone"] = $data_array["client-phone"]; }
		if (empty($data_array["company_name"])) { $data_array["company_name"] = $data_array["client-corporation"]; }
		...
		if ( !empty($data_array["token_rdstation"]) && !( empty($data_array["email"]) && empty($data_array["email_lead"]) ) ) {
			...
			if (in_array ('curl', get_loaded_extensions())) {
				...
				$filename = './logs/'.time();
				file_put_contents($filename.'-rd-response', serialize($ch));
			...
			$filename = './logs/'.time();
			file_put_contents($filename.'-rd-response', 'Data failed validation');
		}
	} catch (Exception $e) { 

		$filename = './logs/'.time();
		file_put_contents($filename.'-rd-response', var_dump($e));
	}
...
function addLeadConversionToRdstationCrmViaWpCf7( $cf7 ) {
	...
	$form_data = $wpcf->get_posted_data();
	addLeadConversionToRdstationCrm($token_rdstation, null, $form_data);

	return true;
}
add_action('wpcf7_mail_sent', 'addLeadConversionToRdstationCrmViaWpCf7', 10, 1);
